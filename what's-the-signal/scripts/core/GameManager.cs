...
public partial class GameManager : Node
{
    ...
    public override void _Ready ()
    {
        ...
        GD.Print( "GameManager: === INIT === autoload up, state=Boot" );
    }

    public void ReportReady ( string system, string detail = null )
    {
        var tail = string.IsNullOrEmpty( detail ) ? "" : $" ({detail})";
        GD.Print( $"GameManager: ready <- {system}{tail} [phase={_state}]" );
    }
    ...
    public async void LoadLevel ()
    {
        ...
        GD.Print( "GameManager: LoadLevel — switching to gameplay scene" );
    ...
    public bool ChangeState ( State newState )
    {
        ...
        var previous = _state;
        _state = newState;
        GD.Print( $"GameManager: === PHASE {newState} === (prev={previous})" );
